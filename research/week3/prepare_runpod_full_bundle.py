"""Create the secret-free, checksum-validated Phase G full GPU bundle."""
...
OUTPUT = ARTIFACTS / "runpod_phase_g_full.zip"
...
def main() -> None:
    ...
    CHECKSUM.write_bytes(f"{archive_sha256}  {OUTPUT.name}\n".encode("ascii"))
    MANIFEST.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "purpose": "phase-g-33-document-full-gpu-run",
                "archive": OUTPUT.name,
                "archive_bytes": OUTPUT.stat().st_size,
                "archive_sha256": archive_sha256,
                "frozen_qa_sha256": sha256(
                    ARTIFACTS / "qa-full" / "qa_frozen.jsonl"
                ),
                "files": records,
                "excludes": [
